...
export default function CandidatesView({ jobs, candidates, interviews, persistCandidates, persistInterviews }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const next = [...candidates, { ...form, id: crypto.randomUUID(), createdAt: new Date().toISOString().split('T')[0] }]
  ...
  if (selected && candidate) {
    ...
    return (
      <div>
        <button onClick={() => { setSelected(null); setShowStatusPicker(false); setShowIvForm(false) }} style={backBtn}>← Zurück zu Bewerber</button>

        {/* Header card */}
        <div style={card}>
          <div style={{ display:'flex', gap:16, alignItems:'flex-start', flexWrap:'wrap' }}>
            <Avatar name={`${candidate.firstName} ${candidate.lastName}`} size={52} />
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap', marginBottom:6 }}>
                <h2 style={{ fontSize:18, fontWeight:600 }}>{candidate.firstName} {candidate.lastName}</h2>
                <Badge status={candidate.status} />
                <button onClick={() => setShowStatusPicker(v => !v)} style={btnSm}>Status ändern</button>
              </div>
              <div style={{ display:'flex', gap:16, flexWrap:'wrap', fontSize:12, color:'#78716c' }}>
                <span>✉ {candidate.email}</span>
                <span>📞 {candidate.phone}</span>
                {job && <span>💼 {job.title}</span>}
              </div>
              {candidate.notes && <p style={{ fontSize:12, color:'#78716c', marginTop:10, padding:'8px 10px', background:'#f5f5f4', borderRadius:8, lineHeight:1.6 }}>{candidate.notes}</p>}
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              <button style={btnSm}>⬆ CV hochladen</button>
              <button style={btnSm}>📷 Foto hinzufügen</button>
            </div>
          </div>
            ...
            <div style={{ marginTop:16, display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              {STATUSES.map(s => (
                <button key={s} onClick={() => handleStatus(s)} style={{
                  padding:'9px 12px', border: candidate.status === s ? '2px solid #378ADD' : '1px solid #e5e5e3',
                  borderRadius:8, background: candidate.status === s ? '#E6F1FB' : '#fff',
        ...
        {civs.length === 0 && !showIvForm && <p style={{ color:'#aaa', fontSize:13 }}>Noch keine Gespräche erfasst.</p>}
        {civs.map(iv => (
          <div key={iv.id} style={{ display:'flex', gap:12, marginBottom:16 }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <span style={{ fontSize:11, padding:'1px 8px', borderRadius:100, background: iv.done ? '#EAF3DE' : '#E6F1FB', color: iv.done ? '#173404' : '#0C447C' }}>{iv.done ? 'Abgeschlossen' : 'Geplant'}</span>
              </div>
    ...
    <div>
      ...
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
        ...
        <tbody>
          ...
          {filtered.length === 0 && <tr><td colSpan={5} style={{ textAlign:'center', padding:24, color:'#aaa' }}>Keine Bewerber in dieser Kategorie</td></tr>}
        </tbody>
      </table>
    </div>
  )
