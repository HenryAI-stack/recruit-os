...
const AV_COLORS = [
  ['#EBF4FF','#1A56DB'],['#ECFDF5','#065F46'],
  ['#FEF3C7','#92400E'],['#F0EEFF','#4C1D95'],['#FEF2F2','#991B1B'],
]
...
function StatCard({ label, value, sub, icon, color = '#1A1A1A' }) {
  return (
    <div style={{ background:'#fff', border:'1px solid #EBEBEA', borderRadius:12, padding:'18px 20px', boxShadow:'0 1px 3px rgba(0,0,0,.04)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
        <span style={{ fontSize:12, fontWeight:600, color:'#999', textTransform:'uppercase', letterSpacing:'.05em' }}>{label}</span>
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Willkommen zurück — Ihre aktuelle Übersicht</p>
        </div>
      </div>

      {/* Stats */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:28 }}>
        <StatCard label="Offene Stellen"    value={openJobs}          sub={`von ${jobs.length} gesamt`}         icon="briefcase" />
        <StatCard label="Bewerber"          value={candidates.length} sub="im aktiven Prozess"                  icon="users" />
        <StatCard label="Gespräche geführt" value={doneIv}            sub={`von ${interviews.length} gesamt`}   icon="calendar" />
        <StatCard label="Ausgewählt"        value={selected}          sub="Kandidaten"                          icon="check" color="#10B981" />
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:18 }}>
        ...
        <div>
          <div className="section-header">
            <span className="section-title">Geplante Gespräche</span>
          ...
          <div style={{ background:'#fff', border:'1px solid #EBEBEA', borderRadius:12, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,.04)' }}>
            {upcoming.length === 0
              ? <p style={{ padding:20, fontSize:13, color:'#bbb', textAlign:'center' }}>Keine geplanten Gespräche</p>
              : upcoming.map((iv,i) => {
                  ...
                  return (
                    <div key={iv.id} style={{ padding:'11px 14px', borderBottom: i<upcoming.length-1 ? '1px solid #F3F3F1' : 'none' }}>
                      ...
                      <div style={{ display:'flex', gap:10, fontSize:11, color:'#aaa', marginTop:3 }}>
                        <span style={{ display:'flex', alignItems:'center', gap:3 }}><Icon name="calendar" size={11} color="#bbb" />{iv.scheduledAt?.replace('T',' ').slice(0,16)}</span>
                        <span>{iv.type}</span>
                        <span style={{ display:'flex', alignItems:'center', gap:3 }}><Icon name="users" size={11} color="#bbb" />{iv.interviewer}</span>
                      </div>
                    </div>
                  )
                })
